...
from simulators import SimGPS, SimPing
...
def load_config_defaults(args) -> dict:
    cfg = {
        ...
        "gps_baud": int(args.gps_baud),
        "ping_port": args.ping_port,
        "ping_baud": int(args.ping_baud),
        "offsets": {
            "forward_m": float(args.ant_forward_m),
            "right_m": float(args.ant_right_m),
        },
    }
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            for k in ("gps_port", "gps_baud", "ping_port", "ping_baud"):
                if k in disk and disk[k] is not None:
                    cfg[k] = disk[k]

            if isinstance(disk.get("offsets"), dict):
                cfg["offsets"]["forward_m"] = float(disk["offsets"].get("forward_m", cfg["offsets"]["forward_m"]))
                cfg["offsets"]["right_m"] = float(disk["offsets"].get("right_m", cfg["offsets"]["right_m"]))
        except Exception:
            pass
            ...
            try:
                ...
                with self.shared.lock:
                    ...
                    if msg.sentence_type == "GGA":
                        ...
                        try:
                            self.shared.gps.fix_quality = int(getattr(msg, "gps_qual", "") or 0)
                        except Exception:
                            self.shared.gps.fix_quality = None
                        try:
                            self.shared.gps.num_sats = int(getattr(msg, "num_sats", "") or 0)
                        except Exception:
                            self.shared.gps.num_sats = None
                        try:
                            self.shared.gps.hdop = float(getattr(msg, "horizontal_dil", "") or 0.0)
                        except Exception:
                            self.shared.gps.hdop = None
                        try:
                            self.shared.gps.alt_m = float(getattr(msg, "altitude", "") or 0.0)
                        except Exception:
                            self.shared.gps.alt_m = None
                        ...
                        self.shared.gps.lon_deg = getattr(msg, "longitude", None)
                        try:
                            self.shared.gps.sog_knots = float(getattr(msg, "spd_over_grnd", "") or 0.0)
                        except Exception:
                            self.shared.gps.sog_knots = None
                        try:
                            self.shared.gps.cog_deg = float(getattr(msg, "true_course", "") or 0.0)
                        except Exception:
                            self.shared.gps.cog_deg = None
...
class PingReader:
    ...
    def _run(self):
        ...
        self.status = f"connected {self.port}@{self.baud}"
        period = 1.0 / max(0.5, float(self.poll_hz))
...
def load_basemaps(path: str) -> dict:
    ...
    for k, v in data.items():
        if ("path" not in v) and ("url" not in v):
            raise ValueError(f"Basemap '{k}' must include either 'path' (offline MBTiles) or 'url' (online XYZ template).")
        v.setdefault("label", k)
        v.setdefault("format", "png")
        v.setdefault("maxZoom", 19)
        v.setdefault("attribution", "")
    return data
...
class TileDownloadManager:
    ...
    def start(self, *, bounds, min_zoom: int, max_zoom: int, name: str, source: str = "osm"):
        ...
        t = threading.Thread(
            target=self._run,
            args=(job_id, bounds, min_zoom, max_zoom, name, source, cancel_ev),
            daemon=True,
        )
        t.start()
        return job_id
    ...
    def _run(self, job_id: str, bounds, min_zoom: int, max_zoom: int, name: str, source: str, cancel_ev: threading.Event):
        ...
        try:
            ...
            sess.headers.update({"User-Agent": "gps-ping-survey/1.0 (offline tile download; local field unit)"})
                ...
                try:
                    ...
                    if r.status_code == 200 and r.content:
                        ...
                        conn.execute(
                            ...
                            (z, x, tms_y, sqlite3.Binary(r.content)),
                        )
                        ok_count += 1
                except Exception:
                    pass
...
def start_web_ui(
    *,
    ...
    sqlite_logger: Optional[SQLiteLogger],
    ...
    offsets_forward_m = float(offsets.get("forward_m", 0.0))
    offsets_right_m = float(offsets.get("right_m", 0.0))

    tile_mgr = TileDownloadManager(
        basemaps_json_path=os.path.abspath(os.environ.get("BASEMAPS_JSON", "basemaps.json")),
            ...
            out.append(
                {
                    ...
                    "maxZoom": int(cfg.get("maxZoom", 19) or 19),
                }
            )
        return jsonify(out)
    ...
    def data():
        ...
        with shared.lock:
            ...
            corr_lat, corr_lon = apply_antenna_offset(
                raw_lat, raw_lon, cog, offsets_forward_m, offsets_right_m
            )
        ...
        return jsonify(
            {
                ...
                "offsets_forward_m": float((config.get("offsets") or {}).get("forward_m", 0.0)),
                "offsets_right_m": float((config.get("offsets") or {}).get("right_m", 0.0)),
                "sim_gps": bool(sim_gps),
                "sim_ping": bool(sim_ping),
    ...
    def set_config_endpoint():
        ...
        offsets_in = config.get("offsets") or {}
        fwd = float(data.get("offsets_forward_m", offsets_in.get("forward_m", 0.0)) or 0.0)
        right = float(data.get("offsets_right_m", offsets_in.get("right_m", 0.0)) or 0.0)

        config["offsets"] = {"forward_m": fwd, "right_m": right}

        if not sim_gps:
            gp = (data.get("gps_port") or "").strip()
            gb = int(data.get("gps_baud") or 9600)
            if not gp:
                return jsonify({"error": "gps_port is required"}), 400
            config["gps_port"] = gp
            config["gps_baud"] = gb
        ...
        if not sim_ping:
            ...
            if not pp:
                return jsonify({"error": "ping_port is required"}), 400
            config["ping_port"] = pp
            config["ping_baud"] = pb
        ...
        if not sim_gps:
            gps_mgr.start(config["gps_port"], int(config["gps_baud"]))
        if not sim_ping:
            ping_mgr.start(config["ping_port"], int(config["ping_baud"]))

        return jsonify({"ok": True, **get_config().json})
    ...
    def export_start():
        ...
        def runner():
            try:
                ...
                base = f"session_{session_id}_{now.strftime('%Y%m%d_%H%M%S_%Z')}_UTC{now.strftime('%z')}"
                outs = []
                ...
                if fmt in ("geotiff", "all"):
                    ...
                    exporters.export_geotiff_depth(
                        sqlite_path,
                        session_id,
                        out_tif,
                        grid_m=grid_m,
...
def main():
    ap = argparse.ArgumentParser(description="GPS + Ping2 survey logger with WebUI (supports simulators).")
    ap.add_argument("--gps-port", default="", help="Serial port for GPS (ignored with --sim-gps)")
    ap.add_argument("--gps-baud", type=int, default=9600)
    ap.add_argument("--ping-port", default="", help="Serial port for Ping2 (ignored with --sim-ping)")
    ...
    ap.add_argument("--ant-forward-m", dest="ant_forward_m", type=float, default=0.0)
    ap.add_argument("--ant-right-m", dest="ant_right_m", type=float, default=0.0)
    ap.add_argument("--sim-gps", action="store_true", help="simulate GPS instead of reading a serial port")
    ap.add_argument("--sim-ping", action="store_true", help="simulate Ping instead of reading a serial port")
    args = ap.parse_args()

    if (not args.sim_gps) and (not args.gps_port):
        raise SystemExit("gps-port is required unless --sim-gps is set")
    if (not args.sim_ping) and (not args.ping_port):
        raise SystemExit("ping-port is required unless --sim-ping is set")
    ...
    sim_gps = SimGPS(shared, hz=max(0.5, float(args.log_hz)))
    sim_ping = SimPing(shared, hz=max(1.0, float(args.ping_hz)))

    if args.sim_gps:
        sim_gps.start()
        gps_mgr.status = "simulated"
    else:
        gps_mgr.start(cfg["gps_port"], int(cfg["gps_baud"]))

    if args.sim_ping:
        sim_ping.start(depth_m=3.0)
        ping_mgr.status = "simulated"
    else:
        ping_mgr.start(cfg["ping_port"], int(cfg["ping_baud"]))
    ...
    web_thread = threading.Thread(
        ...
        kwargs=dict(
            ...
            gps_mgr=gps_mgr,
            ping_mgr=ping_mgr,
            sim_gps=args.sim_gps,
            sim_ping=args.sim_ping,
        ),
        daemon=True,
    )
    web_thread.start()
    ...
    period = 1.0 / max(0.1, float(args.log_hz))
    ...
    try:
        while True:
            ...
            with shared.lock:
                gps_age = now - shared.last_gps_update if shared.last_gps_update else 1e9
                ping_age = now - shared.last_ping_update if shared.last_ping_update else 1e9
                gps_stale = gps_age > float(args.stale_seconds)
                ping_stale = ping_age > float(args.stale_seconds)
                ...
                corr_lat, corr_lon = apply_antenna_offset(
                    ...
                    raw_lon,
                    cog,
                    float(offsets.get("forward_m", 0.0)),
                    float(offsets.get("right_m", 0.0)),
                )
